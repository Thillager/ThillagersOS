...
public class SuperCalculator extends JFrame {
    ...
    private JButton calcBinom, calcNormal, solveBtn, clearBtn, copyBtn, tempumrechBtn, switchThemeBtn, autoAusBtn,
            einheitBtn, prozentBtn, wurzelBtn, extendedBtn, langBtn, speakBtn, simulationBtn, simulationStopBtn, helpBtn, zufallgeoBtn;
    ...
    private void initTexts() {
        ...
        texts.put("zufallgeo", new String[] { "Geogebra Zufallszahl", "Geogebra Random Number" });
    ...
    private void initComponents() {
        ...
        zufallgeoBtn = new JButton();
        styleButton(zufallgeoBtn, new Color(142, 68, 173));
    ...
    private void updateLanguage() {
        ...
        zufallgeoBtn.setText(texts.get("zufallgeo")[idx]);
    }
    ...
    private void setupListeners() {
        ...
        zufallgeoBtn.addActionListener(e -> starteZufallsListe());
    }
    ...
    private JButton[] extendedList() {
        return new JButton[] { tempumrechBtn, einheitBtn, prozentBtn, wurzelBtn, calcBinom, solveBtn, calcNormal,
                simulationBtn, simulationStopBtn, speakBtn, zufallgeoBtn };
    }
    ...
    private void starteZufallsListe() {
    try {
        String text = inputField.getText().toLowerCase().replaceAll("\\s+", "");
        // Regex für zufall(x,y,z)
        Pattern p = Pattern.compile("zufall\\((\\d+),(\\d+),(\\d+)\\)");
        ...
        if (m.find()) {
            ...
            for (int i = 0; i < count; i++) {
                ...
                sb.append(num).append(i < count - 1 ? ", " : "");
            }
            sb.append("}");
            resultField.setText(sb.toString());
        }
    } catch (Exception e) {
        resultField.setText("Error in zufall()");
    }
}
